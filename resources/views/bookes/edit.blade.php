...
@section('body')
<form method="POST" action="{{route('books.update', $value->id)}}">
  @csrf
<div class="mb-3">
    <label for="exampleFormControlInput1" class="form-label">TITLE BOOKS</label>
    <input type="text" class="form-control" name="titlename" value="{{ $value->title }}"  placeholder="name book">
  </div>
  <div class="mb-3">
    <label for="exampleFormControlTextarea1" class="form-label">DESCRIPTION</label>
    <textarea class="form-control" name="descname" rows="3">{{ $value->desc}}</textarea>
  </div>
  <div class="col-auto">
